...
- New clients send `X-Weed-Fitness-Protocol: 2` to opt into exact-unit portions. Requests without that header remain compatible with APK 0.2.2: quick-log items/suggestions and reused assistant ingredients are normalized to the saved food's default serving quantity **and** unit, using its authoritative grams-per-serving. This protects old clients that ignore the unit during logging while OTA adoption is in progress. Existing saved foods are never mutated to achieve compatibility. A food the old client does not include in its supplied context cannot be reconciled remotely; installing the update fixes the client-side unit handling itself.
