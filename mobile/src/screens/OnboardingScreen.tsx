...
export function OnboardingScreen({ date, onComplete }: { date: string; onComplete: (profile: ProfileInput, includeDemo: boolean) => void }) {
  ...
  const preview = useMemo(() => {
    ...
    const base = { displayName: name.trim() || undefined, sex, ageYears: Number(age), heightCm, bodyWeightKg, targetWeightKg, activityFactor: activity, goalMode: mode, goalIntensity: intensity, targetDate, onboardingComplete: true, preferredHeightUnit: heightUnit, preferredWeightUnit: weightUnit };
  ...
  }, [name, sex, age, heightUnit, height, feet, inches, weightUnit, weight, target, activity, mode, intensity, targetDate, date]);
