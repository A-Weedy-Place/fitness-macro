...
export function ProfileScreen({ state, date, status, healthConnect, audioConfigured, appAgentEnabled, initialPanel, activeTheme = loadedTheme, onThemeChange, onSave, onSavePhoto, pinEnabled, onSetLocalPin, onLoadDemo, onExport, onExportDiagnostics, testingTelemetryEnabled, onClearTestTelemetry, onUndoRestore, onImport, onConnectHealth, onOpenHealthSettings, onRefreshIntegrations, onBeforeRestart }: {
  ...
  // Returning to Account leaves local form state mounted, preserving unfinished edits.
  useAndroidBack(() => {
    if (showRestore) { setShowRestore(false); return true; }
    if (showGoalMethod) { setShowGoalMethod(false); return true; }
  ...
  return <Page>
    ...
    {panel ? <Pressable style={styles.back} onPress={() => setPanel(null)}><Ionicons name="chevron-back" size={18} color={colors.actionText} /><Text style={styles.backText}>Account</Text></Pressable> : null}
...
const styles = themedStyles(() => ({
  ...
  back: { alignSelf: 'flex-start', flexDirection: 'row', alignItems: 'center', gap: 1, marginBottom: 8, paddingVertical: 4 }, backText: { color: colors.actionText, fontSize: 12, fontWeight: '900' }, identity: { flexDirection: 'row', alignItems: 'center', gap: 12 }, identityCopy: { flex: 1 }, identityName: { color: colors.ink, fontFamily: 'serif', fontSize: 24, fontWeight: '900' }, identityMeta: { color: colors.actionText, fontSize: 12, fontWeight: '800', marginTop: 4 }, identitySub: { color: colors.muted, fontSize: 10, marginTop: 5 },
  goalKicker: { color: colors.muted, fontSize: 9, fontWeight: '900', letterSpacing: 1.4 }, goalHero: { color: colors.ink, fontFamily: 'serif', fontSize: 25, fontWeight: '900', marginTop: 5 }, goalDetail: { color: colors.muted, fontSize: 10, marginTop: 5 }, explainToggle: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: colors.paper, borderRadius: 12, padding: 11, marginBottom: 10 }, explainToggleText: { color: colors.actionText, fontSize: 10, fontWeight: '900' },
