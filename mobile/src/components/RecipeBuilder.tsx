...
export function RecipeBuilder({ foods, recipe, recipeFood, onSearch, onResolve, onSave, onClose }: { foods: FoodItem[]; recipe?: Recipe; recipeFood?: FoodItem; onSearch: (text: string) => Promise<FoodItem[]>; onResolve: (text: string) => Promise<ResolveResult>; onSave: (input: RecipeInput) => Promise<FoodItem>; onClose: () => void }) {
  ...
  // Keep an in-flight recipe save mounted; otherwise Library handles draft confirmation.
...
const styles = themedStyles(() => ({
  ...
  columns: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 12 }, half: { width: '48.5%' }, summary: { backgroundColor: colors.paper, borderRadius: 13, padding: 10, marginBottom: 9 }, summaryValue: { color: colors.actionText, fontSize: 16, fontWeight: '900' }, summaryText: { color: colors.muted, fontSize: 9, lineHeight: 14, marginTop: 3 }, gap: { height: 6 }
}));
