...
export function FoodDetailSheet({ food, foods, recipes, onClose, onAdd }: { food: FoodItem | null; foods: FoodItem[]; recipes: Recipe[]; onClose: () => void; onAdd?: (food: FoodItem, servingQuantity: number) => void }) {
  ...
  return <Modal visible animationType="slide" onRequestClose={onClose}><SafeAreaView style={styles.root}>
    ...
    <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.macros}><Macro value={total.calories.toFixed(0)} label="Calories" color={colors.sky} large /><Macro value={total.protein.toFixed(1)} label="Protein" color={colors.coral} /><Macro value={total.fat.toFixed(1)} label="Fat" color={colors.gold} /><Macro value={total.carbs.toFixed(1)} label="Carbs" color="#40A873" /></View>
